package com.igor.socialbooks.client;
...
public class LivrosClient {
	...
	public List<Livro> listar() {
		...
		RequestEntity<Void> resquest = RequestEntity.get(URI.create(URI_BASE))
	...
	public String salvar(Livro livro) {
		...
		RequestEntity<Livro> resquest = RequestEntity.post(URI.create(URI_BASE))
				.header("Authorization", credencial).body(livro);

		ResponseEntity<Void> response = restTemplate.exchange(resquest, Void.class);
	...
	public Livro buscar(String uri) {
		...
		RequestEntity<Void> resquest = RequestEntity.get(URI.create(uri))
				.header("Authorization", credencial).build();
